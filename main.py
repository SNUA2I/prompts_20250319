...
--- 페르소나: Role Prompting ------------------------------------------------------
당신은 프로그래밍 튜터입니다. 사용자가 배우고 싶은 프로그래밍 언어나 해결하고 싶은 문제를 알려주면, 단계별로 설명해주세요. 예시 요청: "나는 파이썬으로 간단한 계산기를 만들고 싶습니다." 단계별 안내:
...
출발 공항: <출발 공항 이름> 공항 코드: <출발 공항 코드>
도착 공항: <도착 공항 이름> 공항 코드: <도착 공항 코드>
...
--- 템플릿 사용 (2) ----------------------------------------------------------------------
결과물을 위한 템플릿을 제공해 드리겠습니다. 꺾쇠 괄호 <> 안의 단어가 플레이스홀더입니다.  플레이스홀더를 당신의 결과물로 채워주세요. 제 템플릿의 전체 서식을 그대로 유지해 주세요. 제 템플릿은 다음과 같습니다:
질문: <질문> 대답: <답변>
템플릿을 채울 데이터를 제공받으면 템플릿의 서식에 맞게 대답하세요.
아래 URL에 있는 내용을 사용하여 20개의 질문을 만드세요. https://www.drbworld.com/holding/
...
--- AI를 프로그래밍 할 수 있을까? --------------------------------------------------------
아래 내용을 CSV 형식으로 출력하세요. 최근 발견된 행성 후르츠에서는 많은 과일들이 있습니다. 거기에서 자라는 네오스키즐이라는 과일은 보라색이며 사탕처럼 달콤한 맛이 납니다. 또한 회색빛 파란색의 과일인 로헤클도 있으며, 레몬처럼 매우 시큼합니다. 파운잇은 밝은 녹색이며 단맛보다는 짭짤한 맛이 납니다. 네온 핑크색의 루프노바도 풍부하며, 솜사탕처럼 맛이 납니다. 마지막으로, 옅은 오렌지색을 import streamlit as st
import re
