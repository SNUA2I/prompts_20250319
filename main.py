...
2021년부터 2025년 사이에 출판된 인공지능 분야의 비소설 책 중에서 평점이 4.5 이상인 책을 5권 추천해주세요.

--- 더 명확하고 구체적인 지침 ------------------------------------------------------------
2021년부터 2025년 사이에 출판된 인공지능 분야의 비소설 책 중에서 평점이 4.5 이상인 책을 5권 추천해주세요. 추천 책 정보는 다음과 같이 적어 주세요.
--------------------------------------
제목: <책 제목>
저자: <책 저자>
내용: <책 내용 요약>
...
--- 예시 제공 프롬프트(One-shot & Few-shot) -----------
고객의 피자 주문을 유효한 JSON으로 변환합니다.
...
예:
절반은 치즈와 모짜렐라를, 나머지 절반은 토마토 소스, 햄, 파인애플을 곁들인 라지 피자를 주세요.
{"크기": "라지", "유형": "반반", "재료": [["치즈", "모짜렐라"], ["토마토 소스", "햄", "파인애플"]]}

미디엄 사이즈로 주문하고 싶어요. 왼쪽에는 페퍼로니, 올리브, 양파를 넣고, 오른쪽에는 치즈, 베이컨, 버섯을 넣어주세요.
...
--- 답변 길이 지정 (1) ------------------------------------------------------------------
서울대학교 경영대학은 경영학 교육과 연구를 선도하는 대한민국 최고 수준의 경영대학입니다. 학부 및 대학원, 그리고 경영전문대학원(MBA) 등 다양한 프로그램을 통해 글로벌 리더를 양성하고 있습니다. 세계적 수준의 교수진과 연구 성과, 그리고 활발한 산학협력을 기반으로 혁신적인 경영 지식을 창출하고 확산하고 있습니다. 이러한 서울대 경영대학의 역할과 성과를 바탕으로 홍보 기사를 트윗 길이로 작성하세요.

--- 답변 길이 지정 (2) ------------------------------------------------------------------
서울대학교 경영대학은 경영학 교육과 연구를 선도하는 대한민국 최고 수준의 경영대학입니다. 학부 및 대학원, 그리고 경영전문대학원(MBA) 등 다양한 프로그램을 통해 글로벌 리더를 양성하고 있습니다. 세계적 수준의 교수진과 연구 성과, 그리고 활발한 산학협력을 기반으로 혁신적인 경영 지식을 창출하고 확산하고 있습니다. 이러한 서울대 경영대학의 역할과 성과를 바탕으로 홍보 기사를 약 100 단어로 작성하세요.

--- 답변 길이 지정 (3) ------------------------------------------------------------------
서울대학교 경영대학은 경영학 교육과 연구를 선도하는 대한민국 최고 수준의 경영대학입니다. 학부 및 대학원, 그리고 경영전문대학원(MBA) 등 다양한 프로그램을 통해 글로벌 리더를 양성하고 있습니다. 세계적 수준의 교수진과 연구 성과, 그리고 활발한 산학협력을 기반으로 혁신적인 경영 지식을 창출하고 확산하고 있습니다. 이러한 서울대 경영대학의 역할과 성과를 바탕으로 홍보 기사를 세 문단으로 작성하세요.
...
--- 대안 프롬프트 요청하기 --------------------------------------------------------------
앞으로 내가 작업을 요청할 때마다 다음 절차를 따라 주세요.

1) 내가 하려는 작업의 구성 요소를 먼저 나열하세요.
2) 이 작업을 수행할 수 있는 서로 다른 접근 방법을 몇 가지 제안하세요.
3) 각 접근 방식에 맞는 프롬프트를 직접 작성해 주세요.
4) 마지막으로, 제가 다음 작업에 대해서도 같은 과정을 계속할지 물어봐 주세요.

--- 대안 프롬프트 요청하기 (1) --------------------------------------------------------------
고객 불만 데이터를 분석해 핵심 문제를 요약하는 방법을 설계해 주세요.

--- 대안 프롬프트 요청하기 (2) --------------------------------------------------------------
다음 이메일 대화에서 질문을 자동 감지하고, 각 질문 아래에 발신자별 의견을 정리하는 프롬프트를 작성해 주세요.
...
--- 여행 일정 생성기 (B) --------------------------------------------------------------
기능을 추가하려고 합니다. 출발지와 도착지를 알려주면 출발지와 도착지 사이에 경유할 장소를 포함한 전체 경유지 목록을 제공해 주세요.
...
--- 액션 메뉴 만들기 -----------------------------------------------------------------
내가 입력한 내용을 바탕으로 글머리 번호 개요를 생성한 다음 어떤 작업을 할지 물어봅니다. 내가 "<글머리 번호>"만 입력하면, 해당 글머리 번호를 확장합니다. 내가 "<글머리 번호> 글쓰기"라고 입력하면, 선택한 글머리 번호 <글머리 번호>에 대한 내용을 작성합니다. 최소한 3단락 이상의 내용을 작성합니다. 마지막에는 다음 작업을 물어보세요.

지구 온난화의 위험성

--- 사실 확인 ---------------------------------------------------------------------
내가 입력한 내용을 바탕으로 글머리 번호 개요를 생성한 다음 어떤 작업을 할지 물어봅니다. 내가 "<글머리 번호>"만 입력하면, 해당 글머리 번호를 확장합니다. 내가 "<글머리 번호> 글쓰기"라고 입력하면, 선택한 글머리 번호 <글머리 번호>에 대한 내용을 작성합니다. 최소한 3단락 이상의 내용을 작성합니다. 텍스트를 출력할 때마다 결과물에 포함할 사실 근거를 웹 검색을 통해 찾아서 생성합니다. 사실 근거는 생성한 텍스트의 끝에 출처 URL과 함께 삽입해야 합니다. 사실 근거의 사실 중 하나라도 틀릴 경우 결과물의 진실성을 약화시킬 수 있습니다. 따라서 사실 근거는 근본적인 사실이어야 합니다. 마지막에는 다음 작업을 물어보세요.
...
--- 시맨틱 필터링 -------------------------------------------------------------------
다음 내용을 필터링하여 개인 식별이 가능한 정보 또는 개인을 재식별하는 데 사용될 수 있는 정보를 제거합니다. 정보를 제거한 후 불필요한 재서술이나 일반화는 하지 말고, 최소한의 수정으로 자연스럽게 만듭니다.
...
이 글의 톤과 스타일을 먼저 검토하세요. 이 글이 대화체이고 따뜻한 톤을 목표로 하고 있는지 판단한 뒤, 그 기준에 맞지 않는 부분이 있다면 수정 방향을 제안하세요.

--- 스텝백 프롬프트: Step-back Prompting (2) -------------------------------------------------------------------
이 글의 전체 구조를 먼저 검토하세요. 핵심 메시지가 어디에 위치해 있는지, 문단 간 논리 흐름이 자연스러운지 판단한 뒤, 구조 차원에서 개선이 필요한 지점을 정리하세요.

--- 스텝백 프롬프트: Step-back Prompting (3) -------------------------------------------------------------------
이 글에서 가장 핵심적인 메시지가 무엇인지 먼저 정의하세요. 그 메시지가 독자에게 충분히 드러나고 있는지 평가한 뒤, 필요하다면 강조 방향을 제안하세요.

--- 스텝백 프롬프트: Step-back Prompting (4) -------------------------------------------------------------------
이 글의 대상 독자가 대학생이라고 가정했을 때, 현재 내용과 표현이 적절한지 먼저 평가하세요. 대학생 독자의 관점에서 이해하기 어려운 부분이나 거리감이 느껴지는 요소를 식별하세요.

--- 스텝백 프롬프트: Step-back Prompting (5) -------------------------------------------------------------------
이 글의 결론이 어떤 역할을 수행해야 하는지 먼저 정리하세요. 독자에게 어떤 인식이나 행동을 유도하려는지 판단한 뒤, 현재 결론이 그 목적을 충족하는지 평가하세요.

--- 스텝백 프롬프트: Step-back Prompting (6) -------------------------------------------------------------------
이 글이 해결하려는 근본적인 문제와 목표가 무엇인지 먼저 정리하세요. 그 목표를 기준으로 현재 내용이 적절하게 구성되어 있는지 평가하세요.
...
아래 문제에 대한 해결책을 세 가지 제안해 주세요. 각 해결책은 서로 다른 접근 방식을 사용해야 하며, 아래 제약 요인을 반드시 고려해 주세요.
...
<제약 요인>
지구와 화성 사이의 거리가 매우 멀기 때문에 정기적인 재보급이 어렵다.
</제약 요인>

--- 사고의 나무: Tree of Thoughts (ToT) 2 단계 ---------------
아래 해결책들을 비교 평가해 주세요. 각 해결책에 대해 장점과 단점, 초기 노력, 구현 난이도, 핵심 리스크, 제약 요인과의 적합성을 기준으로 10점 만점 점수와 한 줄 근거를 제시해 주세요.
...
각 해결책에 대해 사고 과정을 심화해 주세요. 아래 항목을 모두 포함해 정리해 주세요.
...
질의 응답 작업을 수행하기 위해 다음과 같이 '생각(Thought)', '행동(Action)', '관찰(Observation)' 단계를 번갈아 순차적으로 행합니다.

- [생각] 단계에서는 현재 상황을 논리적으로 추론합니다.
- [행동] 단계는 다음 세 가지 유형 중 하나가 될 수 있습니다.
1) <search>대상</search>
...
[행동] <finish>직지심체요절은 2권 중 하권만 현존하며, 프랑스 국립도서관에 소장되어 있습니다.</finish>
</예제 2>
...
- 사용자의 답이 맞다면 칭찬을 하고 해당 단어의 **뜻**과 **발음**을 간단히 설명합니다.

3. 해당 단어의 뜻을 설명한 후, 이 단어와 관련이 있는 문장과 재미있는 짧은 이야기를 알려줍니다.
- 해당 단어를 포함한 문장 3개를 만들어 보여주고, 그 뜻을 설명합니다.
- 그 다음, 해당 단어와 관련된 재미있는 이야기를 영어 3-5 문장으로 만들어 보여주고 한국어로 해석을 합니다.

4. 마지막 단계로 해당 단어를 어떤 상황에서 사용할 수 있는지 퀴즈를 통해 사용자에게 물어봅니다.
