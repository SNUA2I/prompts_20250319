    ...
    "프롬프트 기법": [
        {
            ...
            "content": """영화 리뷰를 ‘긍정’, ‘중립’ 또는 ‘부정’으로 분류합니다.
...
Sentiment:"""
        },
        {
            ...
            "content": """고객의 피자 주문을 유효한 JSON으로 변환한다.
...
JSON 응답:"""
        },
        {
            ...
            "content": """당신은 프로그래밍 튜터입니다. 사용자가 배우고 싶은 프로그래밍 언어나 해결하고 싶은 문제를 알려주면, 단계별로 설명해주세요.
...
외환 위기로 우리 회사는 매우 힘든 한 해를 보낼 것 같습니다."""
        ...
        {
            "title": "답변 길이 지정",
            "content": """학생 회장 선거 연설문을 트윗 길이로 작성하세요.
...
2020년부터 2024년 사이에 출판된 인공지능 분야의 비소설 책 중에서 평점이 4.5 이상인 책을 5권 추천해주세요."""
        },
        {
            ...
            "content": """최근 5년 내에 출판된 인공지능 분야의 비소설 책 중에서 평점이 4.5 이상인 책을 5권 추천해주세요.
--------------------------------------
제목: <책 제목>
저자: <책 저자>
내용: <책 내용 요약>
...
평점: <평점>"""
        ...
        {
            "title": "외국어(영어) 문장 수정",
            "content": """사용자의 요청에 다음 사항을 기반해서 답하세요.
...
해석 : [당신의 답변에 대한 한국어 해석을 출력]"""
        ...
        {
            "title": "템플릿 사용",
            "content": """출력물을 위한 템플릿을 제공해 드리겠습니다. 꺾쇠 괄호 <> 안의 단어가 플레이스홀더입니다. 플레이스홀더를 채워주세요. 제 템플릿의 전체 서식을 그대로 유지해 주세요. 제 템플릿은 다음과 같습니다:
...
https://www.drbworld.com/holding/"""
        ...
        {
            "title": "더 나은 질문 만들기",
            "content": """지금부터 내가 질문을 할 때, 더 나은 질문 방식이 있다면 제안하고, 그 질문을 사용할지 여부를 나에게 물어봐 주세요.
...
업무 효율성을 어떻게 높일 수 있을까요?"""
        ...
        {
            "title": "대안 프롬프트 요청하기",
            "content": """내가 작업을 수행하기 위한 프롬프트를 작성해 달라고 요청할 때마다 작업이 무엇인지 나열하고, 작업을 완료하기 위한 대체 접근 방법을 나열한 다음, 각 접근 방식에 대한 프롬프트를 직접 작성하세요. 완료되면 대안을 작성할 다음 프롬프트를 만들 수 있는지 물어보세요.
...
이메일 대화에서 질문을 요약하는 여러 가지 프롬프트와 질문에 대한 다양한 이해 관계자의 관점을 평가하는 프롬프트를 작성합니다."""
        ...
        {
            "title": "질문 받기",
            "content": """나는 수학 공부를 잘 하고 싶어요. 이 목표를 달성하기 위해 나에게 질문을 해 주세요. 충분한 정보를 얻어 나에게 적합한 수학 공부 계획을 제안할 수 있을 때까지 질문을 계속하세요. 정보를 다 모으면 수학 공부 계획을 보여 주세요. 질문을 한 번에 하나씩 하세요. 
...
첫 번째 질문을 시작해 주세요."""
        ...
        {
            "title": "수학 게임",
            "content": """수학 게임을 하겠습니다. 분수와 관련된 질문을 하고 맞출 때마다 점수를 증가시키세요. 답이 틀리면 이유를 알기 쉽게 설명하세요."""
        },
        {
            "title": "AI를 프로그래밍 할 수 있을까?",
            "content": """아래 내용을 CSV 형식으로 출력하세요.
...
제 이름은 홍길동입니다. 저는 현재 인공지능 특강을 듣고 있습니다."""
        ...
        {
            "title": "여행 계획",
            "content": """여행 계획 애플리케이션을 만들려고 합니다. 저는 제 여행에 관해 설명하고 당신은 제가 지나갈 장소에서 할 수 있는 흥미로운 일을 나열합니다. 각 장소에서 며칠 동안 머무를 것인지 알려드리고 가능한 여정을 나열하겠습니다.
...
런던,2->파리,4->로마,5->마드리드,3"""
        ...
        {
            "title": "여행 경유지 추가 기능",
            "content": """기능을 추가하려고 합니다. 출발지와 도착지를 알려주면 출발지와 도착지 사이에 경유할 장소를 포함한 전체 경유지 목록을 제공합니다.
...
런던,1->…->프라하,2"""
        ...
        {
            "title": "개요 확장하기",
            "content": """개요 확장자 역할을 하세요. 제가 입력한 내용을 바탕으로 글머리 번호 개요를 생성한 다음, 어떤 글머리 번호를 확장할지 물어봅니다. 제가 선택한 글머리 번호에 대한 새로운 개요를 만듭니다. 마지막에는 다음에 확장할 글머리 기호를 물어보세요.

지구 온난화의 위험성"""
        },
        {
            "title": "액션 메뉴 만들기",
            "content": """내가 입력한 내용을 바탕으로 글머리 번호 개요를 생성한 다음, 어떤 글머리 번호를 확장할지 물어봅니다. 내가 선택한 글머리 번호에 대한 새로운 개요를 만듭니다. 내가 "<글머리 번호> 글쓰기"라고 입력하면, 선택한 글머리 번호 <글머리 번호>에 대한 내용을 작성합니다. 최소한 3단락 이상의 내용을 작성합니다. 내가 "<글머리 번호>"만 입력하면, 해당 글머리 기호를 확장합니다. 마지막에는 다음 작업을 물어보세요.

지구 온난화의 위험성"""
        },
        {
            "title": "사실 확인",
            "content": """내가 입력한 내용을 바탕으로 글머리 번호 개요를 생성한 다음, 어떤 글머리 번호를 확장할지 물어봅니다. 내가 선택한 글머리 번호에 대한 새로운 개요를 만듭니다. 내가 "<글머리 번호> 글쓰기"라고 입력하면, 선택한 글머리 번호 <글머리 번호>에 대한 내용을 작성합니다. 최소한 3단락 이상의 내용을 작성합니다. 내가 "<글머리 번호>"만 입력하면, 해당 글머리 기호를 확장합니다. 텍스트를 출력할 때마다 결과물에 포함할 사실 세트 생성합니다. 사실 세트는 결과물의 끝에 삽입해야 합니다. 사실 세트는 사실 중 하나라도 틀릴 경우 결과물의 진실성을 약화시킬 수 있는 근본적인 사실이어야 합니다. 마지막에는 다음 작업을 물어보세요.

지구 온난화의 위험성"""
        },
        {
            "title": "역 프롬프트",
            "content": """https://singjupost.com/steve-jobs-how-to-live-before-you-die-2005-speech-full-transcript/
이 글을 읽고 이러한 글을 만들 수 있는 프롬프트를 작성해주세요."""
        },
        {
            "title": "대안 접근 방식 요청하기",
            "content": """지금부터 같은 작업을 수행할 수 있는 다른 방법이 있다면 가장 좋은 대안을 나열하세요. 대안을 비교하고 대조한 다음 어떤 방법을 사용할지 물어보세요.

데이터에 결측치(missing value)가 있어서 분석이나 모델링 전에 처리해야 해요."""
        },
        {
            "title": "시멘틱 필터링",
            "content": """다음 내용을 필터링하여 개인 식별이 가능한 정보 또는 개인을 재식별하는 데 사용될 수 있는 정보를 제거합니다. 정보를 제거한 후 최소한의 수정으로 내용을 자연스럽게 만듭니다.
...
4. 각 단계에 대한 설명과 함께 결과물을 시각화하여 출력합니다.
        ...
        },
        {
            ...
            "content": """주어진 질문에 답하십시오. 당신은 검색에 접근할 수 있습니다.
...
한국에서 유명한 대학 마인크래프트 서버가 있다던데, 그 서버에 대한 소개를 보고서 형태로 작성해주세요. Web Pilot을 이용하세요."""
        ...
        {
            "title": "영어 회화 챗봇",
            "content": """당신은 영어 회화 파트너입니다. 사용자가 대화 연습을 원하는 주제나 상황을 알려주면, 그에 맞는 영어 대화를 시작해주세요. 영어로 말한 뒤 영어의 뜻을 괄호 안에 적어주세요. 만일 사용자의 영어가 부적절하거나 더 나은 표현이 있다면 어떻게 영어로 표현하는지 가르쳐주세요. 그리고 사용자가 영어로 대답을 못하고 한국어로 대답하면 그 대답을 어떻게 영어로 할 수 있는지 알려주세요. 당신의 모든 설명은 한국어로 하세요."""
        },
        {
            "title": "영어 단어 공부 챗봇",
            "content": """당신은 영어 단어를 재미있고 효과적으로 배울 수 있도록 도와주는 친근하고 격려하는 영어 선생님입니다. 
...
if not st.session_state["authenticated"]:
    ...
    for category, prompt_list in prompts.items():
        ...
        for prompt in prompt_list:
            # 간단한 앵커 아이디: 공백은 '-'로 대체
            anchor = prompt["title"].replace(" ", "-")
            st.sidebar.markdown(f"- [{prompt['title']}](#{anchor})")
    
    # -----------------------------
    # 중앙 콘텐츠: 프롬프트 출력
    # -----------------------------
    st.title("프롬프트 엔지니어링 실습 - 프롬프트 모음")
    ...
    for category, prompt_list in prompts.items():
        ...
        for prompt in prompt_list:
            anchor = prompt["title"].replace(" ", "-")
            # HTML 앵커를 사용해 링크 타겟 지정
            st.markdown(f'<h2 id="{anchor}">{prompt["title"]}</h2>', unsafe_allow_html=True)
            st.text_area("", prompt["content"], height=250)
